                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-wrapper">
                                    ...
                                    <input type="tel" v-model="form.telefono" placeholder="4941161315" maxlength="10"
                                        @input="soloNumeros" />
                                </div>
                            </div>
                        </div>
...
const form = ref({
    ...
    correo: '', telefono: '', nacionalidad: '', num_pasaporte: ''
})
...
const inicializarTabla = () => {
    ...
    nextTick(() => {
        tablaInstance = window.$('#tablaPasajeros').DataTable({
            ...
            columns: [
                { data: 'id_pasajeros' },
                { data: null, render: p => `${p.nombre} ${p.primer_apellido}${p.segundo_apellido ? ' ' + p.segundo_apellido : ''}` },
                { data: 'correo' },
...
const guardarPasajero = async () => {
    ...
    if (!regexCorreo.test(form.value.correo.trim())) { error.value = 'El correo no es válido. Usa el formato: nombre@dominio.com'; return }
    const partes = form.value.correo.trim().split('@')
    if (partes[1] && partes[1].split('.').pop().length < 2) { error.value = 'El dominio del correo no es válido.'; return }
    const regexTel = /^\d{10}$/

    if (form.value.correo.includes(' ')) { error.value = 'El correo no puede contener espacios.'; return }
    if (!regexCorreo.test(form.value.correo.trim())) { error.value = 'El correo solo puede tener letras o números antes del @.'; return }
    if (form.value.telefono && !regexTel.test(form.value.telefono.trim())) { error.value = 'El teléfono debe tener exactamente 10 dígitos.'; return }

    error.value = ''
    loading.value = true
    try {
        await actualizarPasajero(pasajeroEditandoId.value, form.value)
        window.Swal.fire({ icon: 'success', title: '¡Actualizado!', text: 'Pasajero actualizado correctamente.', background: '#1a0c10', color: '#f0e8e0', confirmButtonColor: '#c9a84c' })
        cerrarModal()
        await cargarPasajeros()
    } catch (err) {
        error.value = err.response?.data?.message || 'Error al actualizar el pasajero.'
    } finally {
        loading.value = false
    }
}
...
const confirmarEliminar = async (id) => {
    try {
        ...
        const reservasActivas = res.data.filter(r =>
            r.id_pasajero === id && r.estado === 'confirmada'
        )
        if (reservasActivas.length > 0) {
            window.Swal.fire({
                icon: 'warning',
                title: 'No se puede eliminar',
                text: `Este pasajero tiene ${reservasActivas.length} reserva(s) activa(s). Cancélalas primero.`,
                background: '#1a0c10', color: '#f0e8e0', confirmButtonColor: '#c9a84c'
            })
            return
        }
    ...
    const result = await window.Swal.fire({
        title: '¿Eliminar pasajero?', text: 'Esta acción no se puede deshacer.',
    ...
    try {
        await eliminarPasajero(id)
        window.Swal.fire({ icon: 'success', title: 'Eliminado', text: 'El pasajero fue eliminado.', background: '#1a0c10', color: '#f0e8e0', confirmButtonColor: '#c9a84c' })
        await cargarPasajeros()
    } catch (err) {
